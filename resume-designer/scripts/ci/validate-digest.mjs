...
export function validateDigest(text, version) {
  ...
  // model, and "on paper" is a lowercase brand that a writing model will
  // sometimes title-case to "On Paper". Rejecting for that would silently throw
  // away a good digest and publish the raw grouped commit log instead — a worse
  // outcome than a miscased word. Everything else about the format stays strict,
  // because that strictness is what makes prompt-injected output unpublishable.
